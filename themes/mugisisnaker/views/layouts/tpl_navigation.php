...
if(YII::app()->user->getLevel()==1){
 ...
 $this->widget('zii.widgets.CMenu',array(
  ...
  'items'=>array(
    ...
    array('label'=>'Master <span class="caret"></span>', 'url'=>'#','itemOptions'=>array('class'=>'dropdown','tabindex'=>"-1"),'linkOptions'=>array('class'=>'dropdown-toggle','data-toggle'=>"dropdown"), 
      'items'=>array(
        array('label'=>'<span class="label label-info">'.Yii::app()->db->createCommand("SELECT COUNT(id) FROM perusahaan")->queryScalar().'</span> Perusahaan','url'=>array('/perusahaan/admin')),
        array('label'=>'<span class="label label-info">'.Yii::app()->db->createCommand("SELECT COUNT(id) FROM industri")->queryScalar().'</span> Industri','url'=>array('/industri/kelola')),
        array('label'=>'<span class="label label-info">'.Yii::app()->db->createCommand("SELECT COUNT(id) FROM serikat")->queryScalar().'</span> Serikat','url'=>array('/serikat/kelola')),
        array('label'=>'<span class="label label-info">'.Yii::app()->db->createCommand("SELECT COUNT(id) FROM kecamatan")->queryScalar().'</span> Kecamatan','url'=>array('/kecamatan/kelola')),
