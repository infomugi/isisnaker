			...
			<TR>
				...
				<TR>
					...
					$this->widget('ext.multimodelform.MultiModelForm',array(
						...
						'addItemText' => '<div class="btn btn-success btn-flat btn-small btn-sm"><i class="fa fa-plus"></i> Keluarga Tidak Bekerja</div>',
				...
				<script type="text/javascript">
					jQuery(document).ready(function(){
						jQuery("#Umpi_no_kk").keyup(function(event){
							if(event.keyCode == 13){
								jQuery("#Umpi_nama").focus();
							}
						});

						jQuery("#Umpi_nama").keyup(function(event){   
							if(event.keyCode == 13){      
						...
						jQuery("#kd_kecamatan").keyup(function(event){   
							if(event.keyCode == 13){      
								jQuery('#Umpi_kd_desa').focus();
							}
						});

						jQuery("#Umpi_kd_desa").keyup(function(event){   
							if(event.keyCode == 13){      
								jQuery('#Umpi_alamat').focus();
							}
						});

						jQuery("#Umpi_alamat").keyup(function(event){   
							if(event.keyCode == 13){      
								jQuery('#Umpi_rt').focus();
							}
						});		

						jQuery("#Umpi_rt").keyup(function(event){   
							if(event.keyCode == 13){      
								jQuery('#Umpi_rw').focus();
							}
						});									

						jQuery("#Umpi_rw").keyup(function(event){   
							if(event.keyCode == 13){      
								jQuery('#Umpi_telponrumah').focus();
							}
						});
